...
const appRoot = __dirname.endsWith(`${path.sep}dist`) ? path.resolve(__dirname, '..') : __dirname;
...
class SimpleSqliteLikeDb {
  ...
  prepare(sql: string) {
    return new SimpleSqliteLikeStatement(this, sql);
  }
...
class SimpleSqliteLikeStatement {
  constructor(
    private readonly db: SimpleSqliteLikeDb,
  ...
  run(...args: any[]) {
    ...
    if (/^DELETE\s+FROM\s+/i.test(sql)) {
      ...
      if (sql.toLowerCase().includes('where')) {
        ...
        const target = args[0];
        if (columnName) {
          const table = this.db.ensureTable(tableName);
          const nextRows = table.filter((row) => row[columnName] !== target);
          table.splice(0, table.length, ...nextRows);
        }
      } else {
        this.db.ensureTable(tableName).length = 0;
      }
    ...
    if (/^INSERT\s+INTO\s+/i.test(sql)) {
      ...
      const columnMatch = sql.match(/\(([^)]+)\)\s*VALUES\s*\(([^)]+)\)/i);
      const conflictMatch = sql.match(/ON\s+CONFLICT\(([^)]+)\)\s+DO\s+UPDATE\s+SET\s+(.+)$/i);
      const colNames = columnMatch?.[1]?.split(',').map((part) => part.trim().replace(/`/g, '')) ?? [];
      const row: Record<string, any> = {};
      colNames.forEach((key, index) => {
        row[key] = args[index];
      });

      const targetKey = conflictMatch?.[1]?.trim().replace(/`/g, '') ?? null;
      const table = this.db.ensureTable(tableName);
      if (targetKey) {
        const existingIndex = table.findIndex((entry) => entry[targetKey] === row[targetKey]);
        if (existingIndex >= 0) {
  ...
  get(...args: any[]) {
    ...
    const idLookupMatch = sql.match(/^SELECT\s+id\s+FROM\s+([A-Za-z0-9_]+)\s+WHERE\s+name\s*=\s*\?/i);
    if (idLookupMatch) {
      const tableName = idLookupMatch[1];
      const table = this.db.ensureTable(tableName);
      const row = table.find((entry) => entry.name === args[0]);
      return row ? { id: row.id } : undefined;
    }

    const tableMatch = sql.match(/^SELECT\s+\*\s+FROM\s+([A-Za-z0-9_]+)/i);
    const tableName = tableMatch?.[1];
    if (!tableName) {
      return undefined;
    }
    ...
    if (sql.toLowerCase().includes('where token = ?')) {
      return table.find((row) => row.token === args[0] && row.revoked === 0 && new Date(row.expires_at).getTime() > new Date(args[1]).getTime()) ?? undefined;
    }

    if (sql.toLowerCase().includes('where u.id = ?') || sql.toLowerCase().includes('where u.id = ?')) {
      const userId = args[0];
      const user = table.find((row) => row.id === userId);
      if (!user) {
        return undefined;
      }
      const roleTable = this.db.ensureTable('admin_roles');
      const role = roleTable.find((row) => row.id === user.role_id);
      return {
        ...user,
        role_name: role?.name,
    ...
    if (sql.toLowerCase().includes('where')) {
      const whereMatches = sql.match(/WHERE\s+(.+)$/i)?.[1];
      if (whereMatches) {
        const clauses = whereMatches.split(/\s+AND\s+/i);
        return table.find((row) => {
          return clauses.every((clause) => {
            const comparable = clause.match(/([A-Za-z0-9_]+)\s*=\s*\?/i);
            if (comparable) {
              return row[comparable[1]] === args[clauses.indexOf(clause)];
            }
            const gt = clause.match(/([A-Za-z0-9_]+)\s*>\s*\?/i);
            if (gt) {
              return Number(row[gt[1]]) > Number(args[clauses.indexOf(clause)]);
            }
            return true;
          });
        }) ?? undefined;
  ...
  all(...args: any[]) {
    ...
    if (sql.toLowerCase().includes('order by submitted_at desc')) {
      rows.sort((a, b) => String(b.submitted_at ?? '').localeCompare(String(a.submitted_at ?? '')));
    } else if (sql.toLowerCase().includes('order by date desc')) {
      rows.sort((a, b) => String(b.date ?? '').localeCompare(String(a.date ?? '')));
    } else if (sql.toLowerCase().includes('order by name asc')) {
...
export async function startServer() {
  const isProductionRuntime = process.env.NODE_ENV === 'production' || !process.env.ELECTRON_START_URL;
